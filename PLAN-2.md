...
- Domain use-cases (pure TS, no React/Expo, unit-tested — these are the seam Session 18 also calls): `serializeBackup(readings, settings) → BackupFile`; `parseBackup(unknown) → BackupFile` (validate `app` tag + `schemaVersion`, narrow with type guards, never `any`); `migrateBackup(BackupFile) → BackupFile` (forward-migrate older `schemaVersion` to current before apply); `applyBackup(BackupFile, repos)` — **replace** semantics (clear + insert inside one transaction), each reading re-validated (20–600 warn-only rule preserved; garbage rows skipped + counted, never abort).
- Data layer `BackupService`: **Export** → `serializeBackup` → write `sugar-backup-YYYY-MM-DD.json` to cache dir (`expo-file-system`) → `expo-sharing` share sheet (user picks Google Drive / iCloud Files / Files / Zalo / email — we integrate no cloud SDK; the OS sheet gives Drive + iCloud for free, cross-platform). **Import** → `expo-document-picker` (`application/json`) → read → `parseBackup` → `migrateBackup` → confirm overwrite ("Dữ liệu hiện tại trên máy sẽ được thay bằng bản sao lưu này") → `applyBackup` → reset zustand → success ("Đã khôi phục {n} chỉ số").
- **OS auto-backup for free (silent):** ensure the SQLite file is NOT excluded from iCloud (iOS) / Android Auto Backup so users who never tap Export still get in-ecosystem protection. Verify the db path isn't in a no-backup dir.
- Settings → "Sao lưu dữ liệu" section (all modes, signed-out fine): "Xuất tệp sao lưu" + "Khôi phục từ tệp" + "Lần sao lưu gần nhất: {ngày}" nudge (`lastLocalBackupAt` settings key). Caring copy, i18n vi + en.
- **Never gated** — money-principle #1. `backup_enabled` analytics event (Session 15) fires here too (anonymous, no glucose).
- Version rule: `parseBackup` **rejects a file whose `schemaVersion` is newer than the app** ("Tệp này từ phiên bản Sugar mới hơn — hãy cập nhật app") — mirror of Session 18's cloud rule so both paths behave identically.

**Accept:** Export → wipe (Delete All Data) → Import the file = byte-identical readings + settings; import of a hand-edited older-`schemaVersion` file migrates forward; newer-version file rejected with update message; malformed/garbage rows skipped + reported, never crash; works fully signed-out; `serializeBackup`/`parseBackup`/`migrateBackup`/`applyBackup` unit-tested (round-trip, version up-migration, rejection, bad-row skip) against in-memory SQLite; `tsc` + tests green. Commit: `feat: free local json backup and restore`
...
- **Auto backup on foreground** when `isPro && session && now − lastCloudBackupAt > 7 days` — no background task, zero battery. (This same foreground ping also keeps the free-tier project from pausing once real Pro users exist — the 7-day-idle pause only bites pre-launch.)
- **Restore:** "Sao lưu gần nhất: {ngày} · {size}" + "Khôi phục về máy này" → overwrite warning → download JSON → `parseBackup` (validates `app` tag + version) → `migrateBackup` → **reject if `schemaVersion` newer than app** ("hãy cập nhật app trước") → `applyBackup` (transactional clear+insert, older files auto-migrate) → reset zustand → success screen. Identical restore path to local import — cloud only differs in *where the bytes come from*.
- Gate: whole feature Pro (`backup_gate`); NOW add "**Tự động sao lưu đám mây & đồng bộ nhiều máy**" to `PRO_BENEFITS` (Pro sells the *automatic + cross-device* layer; the manual local file itself is free — Session 17.5). Privacy policy + Data Safety updated ("dữ liệu chỉ rời máy khi mẹ bật sao lưu đám mây…"); client-side E2E encryption deliberately deferred (lost-key trap), trade-off documented.
- **RLS is the critical setting** — each user reads only `auth.uid()/` objects and their own `backup_meta` row; verify with a cross-user 403 check.
...
- **Session 24 debug — "There was a credentials issue" on real device (internal test):** real adapter IS live now (5 anon customers in RC dashboard), but purchase (Google test card) charged yet app stayed locked → tapping buy again → Google `ITEM_ALREADY_OWNED` ("Bạn đã sở hữu mặt hàng này"). Decisive evidence: RC → Customers all "-" (0 validated transactions) + RC app settings "⚠ Credentials need attention". View-details breakdown pinned the exact gap: ✅ read in-app catalog, ✅ read subscription catalog, ❌ **Can validate Google Play subscription purchases**. Root cause = RC↔Play **purchase-validation** credential not yet effective. Note: Pub/Sub Admin (line above) only covers real-time notifications, NOT validation. Fixes applied in Play Console: (a) Google Play Android Developer API already enabled in `sugar-play`; (b) account-level "View financial data, orders…" + "Manage orders and subscriptions" granted; (c) **service account had NO app-level access — "App permissions" tab was empty → added app Sugar** (the last missing piece the RC tip "grant app access" pointed to). Financial/validate permission propagates slowest (Google: up to 24–36h); catalog-read checks go green first. **RESOLVED next morning:** after the app-level access propagated overnight, RC's "Can validate Google Play subscription purchases" went ✅ (all 3 green), and a real internal-test purchase succeeded end-to-end — Pro unlocked, transaction visible in RC Customers. No app rebuild was needed (credential fix is server-side; the installed build already had the real adapter).
...
_Bước 7 — **COMPLETE 2026-07-26**. Every acceptance item passed on a real device against the real RevenueCat
adapter. Three defects were found and fixed along the way (offline coupling, support code, restore error handling);
the `.aab` submitted in Bước 8 must therefore be built from **`ff00523` or later** — the tested build predates the
restore fix._
...
- [x] Empty Restore — paywall → "Khôi phục giao dịch" while the account owned nothing → "no purchases found". **Verified 2026-07-26**, the only window in which this button was ever testable (the earlier reinstall auto-transferred, so the app was already Pro).
- [x] Free-tier gates against the **real** entitlement, not the dev adapter (first time): report gate, watermark, CSV lock, "Theo bữa" lock. **Verified 2026-07-26.**
...
_Bước 8 — in progress (2026-07-26). Full sheet: `docs/plans/2026-07-26-session-23-buoc-8-production.md`._

- **Artifact settled: no rebuild.** `eas build:list` shows build **#7 = versionCode 7, commit `1dc26e1` (HEAD)** — it already carries `ff00523`, so the "must build from ff00523 or later" note is satisfied by a build that already exists. The device-tested build was **#6** (`338f86e`), so #7 has one untouched delta (paywall `catch` + 2 i18n strings + a hook test) → upload #7 to Internal testing, 2-minute smoke (Pro persists, price shows, Restore alerts on a non-owning account, release details reads versionCode 7), then Promote.
- **The privacy policy was factually wrong and had to be fixed first (this commit).** The live page claimed *"does not collect, transmit to any server, or share"* and PRD §299 promised "no third-party SDKs that phone home" — false since `react-native-purchases` entered the build: the RC SDK sends purchase history + the anonymous app-user id + IP to RevenueCat. Google cross-checks Data Safety against the policy, so declaring purchases against a "we collect nothing" page would have been a self-inflicted contradiction. `docs/privacy.html` (EN+VI) now scopes §1 to *health* data, adds **§5 Purchases** (what RC receives, what it never receives, encryption in transit, deletion via the support code, Google keeps its own record) and **§6 Usage statistics** (states plainly that nothing is sent today). **Needs `git push` to `main`** — Pages serves from `main /docs`.
- **Data Safety answer, verified against RevenueCat's own Play guidance:** collect = **Yes** → **Financial info → Purchase history**, Collected (not Shared — RC is a service provider), not ephemeral, Required, purposes **App functionality + Analytics**; encrypted in transit = Yes; deletion requests = Yes. Session 14's "Data Safety = No" is now marked stale in that guide.
- **Deliberately NOT declaring analytics yet** (differs from the initial framing of Bước 8): `src/data/analytics.ts` is still a `__DEV__`-only `console.log` no-op, there is no `@aptabase` dep, and `rg` confirms **zero network calls in the app besides the RC SDK**. Declaring collection that does not happen is inaccurate in the opposite direction and weakens the store's Data safety card — which is a selling point here. When Aptabase is wired: add **App activity → App interactions** (Collected, *Optional* — the opt-out exists) and update policy §6. Data Safety edits need no new release.
- **Health & fitness stays "not collected"** and that is correct, not a loophole: Data Safety covers data that leaves the device, and readings only leave via a **user-initiated** export/share (Play's explicit exemption).
- Remaining, all browser-side: push Pages → smoke #7 → App content forms (§3 of the sheet) → store listing + 6 screenshots + feature graphic → Promote to Production, VN enabled, 100% rollout. Then one **real** (non-license-tester) purchase to prove production billing, refunded from RevenueCat first.

Checklists: `docs/plans/2026-07-26-session-23-buoc-8-production.md` (Bước 8), `docs/plans/2026-07-25-session-23-buoc-7-acceptance.md` (Bước 7 results), `docs/plans/2026-07-18-session-23-admin-handoff.md` (manual steps).
...
- **Free/Pro matrix:** Free = unlimited logging/history/ranges/reminders/basic trends/weekly summary + **first PDF (watermarked)** + **local backup/restore (JSON export/import, no account)**. Pro = unlimited PDF + no watermark, **CSV export** *(supersedes v1.1 core-free CSV; pre-launch, no user loses anything)*, "Theo bữa" analysis, **automatic cloud backup + cross-device restore**, (no-ads only if Session 19 activates).
- **Backup format unified on versioned JSON** (`schemaVersion` we own; serialize domain models, not the SQLite file) — one serializer/restore path shared by free local (S17.5) and Pro cloud (S18).
- **Auth & backup partially un-deferred:** free local JSON backup/restore needs no account (S17.5). Supabase anonymous-first auth (Google now; Apple at iOS launch per 4.8; secure-store sessions; in-app account deletion + web deletion-request URL) + single-slot **Pro cloud** backup/restore (private bucket + RLS). **Two-way real-time sync stays deferred** ("cross-device" = restore-latest, not live merge.)
...
- **Stories 61–73:** 61 buy Pro · 62 restore purchases · 63 report gate from 2nd + watermark removal · 64 Theo bữa (Pro) · 65 Google sign-in (Apple at iOS) · 66 in-app account deletion · 67 cloud backup (Pro) · 68 restore to new device · 69 weekly summary · 70 postpartum + OGTT · 71 supplies/affiliate · **73 local JSON backup/restore (free, no account — S17.5)**. *(72 ads — only if Session 19 passes its decision gate.)*
